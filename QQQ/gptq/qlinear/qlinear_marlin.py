...
class QuantLinear(nn.Module):
    ...
    def pack(self, linear, scales, int8_scales=None):
        ...
        # w = torch.clamp(w, -(maxq + 1), maxq)
        # NOTE(zhangying): div 2 ** (8 - self.bits)) to deal with right_shift in unpacking
        if self.group_size != self.infeatures:
            int8_scales = int8_scales.reshape(1, -1).to(dtype=torch.half)
            s = (
                s.reshape(-1, self.outfeatures) / (int8_scales * (2 ** (8 - self.bits)))
            ).to(dtype=torch.half)
            ...
            # FIXME(zhangying): int8_scales use _scale_perm or _scale_perm_single?
            int8_scales = int8_scales.reshape((-1, len(_scale_perm_single)))[
                :, _scale_perm_single
            ]
            int8_scales = int8_scales.reshape((-1, self.outfeatures)).contiguous()
        else:
            s = (s / (2 ** (8 - self.bits))).reshape((-1, len(_scale_perm_single)))[
                :, _scale_perm_single
            ]
        ...
        if self.group_size != self.infeatures:
            self.int8_scales[:, :] = int8_scales.to(self.int8_scales.device)
    ...
    def dynamic_quant(self, x: torch.Tensor):
        quant_scale = x.abs().max(dim=-1, keepdim=True)[0].div(127.0).to(x.dtype)
    ...
    def forward(self, A):
        ...
        if self.wf.device != self.B.device:
            self.wf = self.wf.to(self.B.device)

        # unpack weight
        weight = torch.bitwise_right_shift(
            torch.unsqueeze(self.B, 2).expand(-1, -1, 32 // self.bits),
            self.wf.unsqueeze(0),
        ).to(torch.int8)
        ...
        weight = weight.reshape((self.infeatures, self.outfeatures))
        if self.group_size != self.infeatures:
            int8_scales = self.int8_scales.reshape(-1, len(_scale_perm_single))[
                :, _scale_perm_single_inv
            ].reshape(1, -1)
            scales = self.s.reshape(-1, len(_scale_perm))[:, _scale_perm_inv].reshape(
                1, -1
            )
            weight = (
                weight.reshape(-1, self.group_size, self.outfeatures)
                .permute(1, 0, 2)
                .reshape(self.group_size, -1)
            )
            weight = weight * scales
            weight = weight.round().clamp(-128, 127).to(torch.int8)
            weight = (
                weight.reshape(self.group_size, -1, self.outfeatures)
                .permute(1, 0, 2)
                .reshape(self.infeatures, self.outfeatures)
            )
        else:
            scales = self.s.reshape(-1, len(_scale_perm_single))[
                :, _scale_perm_single_inv
            ].reshape(1, -1)

        # quant activation
        quant_A, A_scales = self.dynamic_quant(A)
        ...
        self.i8cugemm.linear_a8_w8_o32_(quant_A, weight.t().contiguous(), C)
        ...
        weight_scales = (
            ...
            if self.group_size == self.infeatures
            else int8_scales.to(torch.float32)
        )
        dequant_scales = A_scales * weight_scales

        C = (C * dequant_scales).to(org_dtype)
