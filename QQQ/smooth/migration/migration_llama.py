...
import QQQ.smooth.models.llama as llama
...
class MigratorBase(nn.Module):
    ...
    def qkv_function(self, input, weight):
        ...
        q, k = llama.apply_rotary_pos_emb(
            q,
            k,
            cos,
            sin,
        )
        k = llama.repeat_kv(k, self.extra_dict["num_key_value_groups"])
        v = llama.repeat_kv(v, self.extra_dict["num_key_value_groups"])
